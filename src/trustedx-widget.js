...
class EnvironmentSelector extends UIComponent {
  ...
  render() {
    ...
    this.element = this.createElement('div', 'trustedx-env-selector', `
...
class LoginButton extends UIComponent {
  ...
  render() {
    this.element = this.createElement('button', 'trustedx-login-btn', 'Se connecter');
...
class LoadingSpinner extends UIComponent {
  ...
  render() {
    this.element = this.createElement('div', 'trustedx-loading', `
    ...
    // Add CSS animation
    if (!document.getElementById('trustedx-spinner-styles')) {
      const style = document.createElement('style');
      style.id = 'trustedx-spinner-styles';
      style.textContent = `
        @keyframes spin {
...
class ErrorDisplay extends UIComponent {
  ...
  render() {
    this.element = this.createElement('div', 'trustedx-error');
    this.element.style.cssText = `
      display: none;
      padding: 12px;
      margin: 10px 0;
...
class UIManager {
  ...
  createMainContainer() {
    ...
    this.mainElement.className = 'trustedx-widget';
    ...
    title.textContent = 'Authentification TrustedX';
    title.style.cssText = `
...
// Widget Factory
// =======================
class TrustedXWidgetFactory {
  static create(config = {}) {
    return new TrustedXAuthWidget(config);
  }

  static createMultiple(configs) {
    return configs.map(config => new TrustedXAuthWidget(config));
  }
...
class DebugPlugin {
  ...
  addDebugPanel() {
    ...
    debugPanel.id = 'trustedx-debug-panel';
  ...
  log(message, data) {
    ...
    console.log(`[TrustedX Debug] ${message}`, data);
  }
...
class EnhancedTrustedXAuthWidget extends TrustedXAuthWidget {
  constructor(config = {}) {
    super(config);
...
if (typeof window !== "undefined") {
  // Main exports
  window.TrustedXAuthWidget = TrustedXAuthWidget;
  window.EnhancedTrustedXAuthWidget = EnhancedTrustedXAuthWidget;
  window.TrustedXWidgetFactory = TrustedXWidgetFactory;
  ...
  window.createTrustedXWidget = (config) => {
    return TrustedXWidgetFactory.create(config);
  };
}